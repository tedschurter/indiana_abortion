---
title: "<span style='font-size: 27px'>Analysis of Indiana Terminated Pregnancy Reports 2014 to 2021 </span>"
author: "<span style = 'font-size: 16px'>by: Ted Schurter </span>"
date:   "<span style = 'font-size: 13px'>June 23, 2023 </span>"
output:
  html_document:
...
packages <- c("tidyverse", "flexdashboard", 
              "crosstalk", "leaflet", "DT", "jsonlite", "sf","tigris", "english")
...
t_theme <- function(base_size = 10){
    ...
    theme( 
      ...
      plot.title = 
        element_textbox_simple(size = rel(1.5), color = "gray25", 
                              hjust = 0,lineheight = 1, family = "serif",
                              face = "plain", margin = unit(c(0, 0, 6, 0), "pt")),
      plot.subtitle = 
        element_textbox_simple(size = rel(1.1), color = "gray40", hjust = 0,
                              lineheight = 1,margin = margin(0.2, 0, .2, 0, 
                                                             unit = "cm"),),
...
#### Indiana was the first state to pass a [new abortion law](https://iga.in.gov/laws/2022/ic/titles/16#16-34) after the U.S. Supreme Court [struck down](https://www.supremecourt.gov/opinions/21pdf/19-1392_6j37.pdf) Roe v. Wade on June 24, 2022. Passed six weeks after the high court’s Dobbs v. Jackson Women's Health Organization decision, Indiana's law prohibits abortion except in cases of incest or rape, fatal fetal anomaly, or to save or preserve the life of the mother. The ban was short-lived as judges in [two](https://thehill.com/homenews/state-watch/3760751-second-judge-blocks-indiana-abortion-ban/) separate lawsuits blocked it while the cases are litigated.

#### While those cases proceed, abortion remains legal up to 22 weeks of gestational age. Though Indiana law cites post-fertilization age, most analyses, including that by the Indiana Department of Health (IDH), instead use gestational age, which is approximately two weeks greater than post-fertilization age. Indiana's [Terminated Pregnancy Reports](https://www.in.gov/health/vital-records/vital-statistics/terminated-pregnancy-reports/), published annually by the IDH, outline the details of abortion in the state. If the ban is upheld, they will also serve as a historical record, illuminating the context of abortion and revealing where it was most common and among what populations.

#### If Indiana’s abortion ban is overturned, the pending outcome of federal lawsuits may impact its practice within the state and beyond. U.S. District Judge Matthew Kacsmaryk [ordered](https://www.reuters.com/world/us/texas-judge-suspends-approval-abortion-pill-what-happens-next-2023-04-08/) in April 2023 a hold on federal approval of the drug mifepristone, one of two drugs commonly used in medication abortions, while the case moves forward challenging its FDA approval. The U.S. Supreme Court ultimately issued a [stay](https://www.google.com/url?sa=t&rct=j&q=&esrc=s&source=web&cd=&cad=rja&uact=8&ved=2ahUKEwjavKO8u9T_AhVMkIkEHf6CC-kQFnoECBQQAQ&url=https%3A%2F%2Fapnews.com%2Farticle%2Fsupreme-court-abortion-pill-mifepristone-access-f781488016640bf571faf36096339ea4&usg=AOvVaw2nbN4gg6wM_CX2hSpwxF_M&opi=89978449) on that decision, preserving access to the drug while the case is decided by the Fifth Circuit Court. 

#### Resolution regarding mifepristone's legality is complicated by another [ruling](https://www.google.com/url?sa=t&rct=j&q=&esrc=s&source=web&cd=&cad=rja&uact=8&ved=2ahUKEwiHjOvVu9T_AhWtjIkEHcXiBTcQFnoECB8QAQ&url=https%3A%2F%2Fwww.atg.wa.gov%2Fnews%2Fnews-releases%2Fag-ferguson-full-protections-mifepristone-access-remain-intact-18-states&usg=AOvVaw09xI-5FsJD_dHYDS2Stx8D&opi=89978449) issued the same day by Judge Thomas O. Rice, District Judge for the Eastern District of Washington, that prohibited the FDA from restricting access to the drug. Rice’s order is limited to Washington, D.C. and the 17 states involved in the case. The competing orders may need to be resolved by the U.S. Supreme Court.<br>

#### If mifepristone's FDA approval is revoked, and the Indiana ban is overturned, medication abortions in the state would be limited to the use of misoprostol. Although not as [effective](https://www.ipas.org/clinical-update/english/recommendations-for-abortion-before-13-weeks-gestation/medical-abortion/safety-and-effectiveness/) as the combination of mifepristone and misoprostol, abortions using just misoprostol, which induces cramping and bleeding to empty the uterus and are common in places without access to mifepristone, could continue.
...
#### Abortions are often classified as either medical or surgical. Medical abortions, sometimes referred to as medication abortions, use medication to end a pregnancy and have become the most common, growing an average `r round(sum(med_cng$pct_cng, na.rm=T)/length(proc_tot$year),2)`% annually. Since the FDA [expanded](https://www.accessdata.fda.gov/drugsatfda_docs/label/2016/020687s020lbl.pdf) the approved use of mifepristone from seven weeks of pregnancy to 10 weeks in 2016, medication abortions have increased `r paste0(100*round((med_cng$Count[med_cng$year == max(med_cng$year)]-med_cng$Count[med_cng$year == 2016]) /med_cng$Count[med_cng$year == max(med_cng$year)],2),"%")` and become the dominant method.
...
# slightly darker medical abortion color for text
rx_c_txt <- "#91adc3" 
...
surg_def_lab <- paste0(
  ...
  <span style = 'color:",procedure_colors$'Suction Curettage',";'>surgical abortions</span>
  <span style = 'color:",body_text,";'>including</span>
  <span style = 'color:",procedure_colors$'Suction Curettage',";'>suction curettage</span>")

# medical line label
med_lab <- paste0(
  "<span style = 'color:",rx_c_txt,";'>Medical<br>abortions</span>")
...
# all surgical label
surg_lab <- paste0(
  "<span style = 'color:",procedure_colors$'Suction Curettage',";'>Surgical<br>abortions</span>")

# non sc surgical types label
non_sc_surg <- paste0(
  "<span style = 'color:",procedure_colors$'Suction Curettage',";'>All surgical abortions</span>
...
pct_title <- paste0(
  "<span style = 'color:",rx_c_txt,";'>Medical</span>
  abortions grew on average</span>
  <span style = 'color:",rx_c_txt,";'>",
  paste0(round(sum(med_cng$pct_cng, na.rm=T)/length(proc_tot$year),2),'%'),"</span>
  annually from 2014 to 2021 and eclipsed</span>
  <span style = 'color:",procedure_colors$Surgical,";'>surgical</span>
  abortions, which declined on average</span>
  <span style = 'color:",procedure_colors$Surgical,";'>",
  paste0(abs(round(sum(surg_cng$pct_cng, na.rm=T)/length(proc_tot$year),2)),"%"),
  "</span>
  annually, as the dominant abortion procedure in 2019. The rate for <span style 
...
subtitle <- paste0(
  "Indiana consolidated all distinct</span><span style = 'color:",procedure_colors$Surgical,";'> surgical</span> abortions into one category in 2018.<br></span>")
...
### Chart
```{r proc_plot, fig.width=10, fig.alt = paste0("Line graph showing medical, surgical and all abortions in Indiana from 2014 to 2021 where all abortions increased at ", round(sum(med_cng$pct_cng, na.rm=T)/length(proc_tot$year),2),"% and medication abortions grew ", round(sum(med_cng$pct_cng, na.rm=T)/length(proc_tot$year),2),"%.")}
ggplot()+
  ...
  geom_segment(aes(x=2014, xend=2021,
                   y = 2000, yend=2000),
               size=.3, color="gray 80", linetype = "dotted")+
  geom_segment(aes(x=2014, xend=2021,
                   y = 0, yend=0),
               size=.1, color="light gray")+
  geom_segment(aes(x=2014, xend=2021,
...
annotate("text_box", x=2021, y = proc_tot$total[proc_tot$year == "2021"],
         label = "All abortions", color = "gray", lineheight = 1,
  ...
  annotate("text_box", x=2021, y = pull(proc_19_21[proc_19_21$Procedure =="Surgical" & 
           ...
           width = unit(1.5, "inch"), hjust = 0, vjust = .75, box.color=panel_c, 
           fill = panel_c, size = 4)+
  # medical label  
...
t_theme()+
  ...
  labs(
    ...
    alt = paste0("Line graph showing medical, surgical and all abortions in Indiana from 2014 to 2021 where all abortions increased at", round(sum(med_cng$pct_cng, na.rm=T)/length(proc_tot$year),2),"% and medication abortions grew", round(sum(med_cng$pct_cng, na.rm=T)/length(proc_tot$year),2),"%."))
...
datatable(rbind(med_cng, surg_cng) %>% select(-pct_cng) %>% 
            pivot_wider(names_from = Procedure,
                        values_from = Count) %>% 
            mutate(Total = Medical+Surgical),
          colnames = c("Year", "Medical\nabortions", "Surgical\nabortions", "Total\nabortions"),
          caption = htmltools::tags$caption(
            style = 'caption-side: top; text-align: center;',
            "Abortions in Indiana by year and procedure type 2014 to 2021.", style = "color:black"),
          rownames=FALSE, 
          filter = "top", 
  ...
  formatRound(c("Medical", "Surgical", "Total"), digits = 0)
...
#### Surgical abortion encompasses many procedures, sometimes called different names, used at different gestational ages and with varying degrees of complexity. The two surgical methods explicitly defined in the [reports](https://www.in.gov/health/vital-records/files/2017-Indiana-Terminated-Pregnancy-Report.pdf#page=30) are dilation and evacuation and suction curettage. They are differentiated by the methods used and the gestational ages when they occur. Like suction curettage, dilation and evacuation abortions utilize suction to empty the uterus but also tools like forceps and a curette; they occur later in a pregnancy, typically midway during the second trimester. Between 2014 and 2018, when Indiana stopped listing them separately, `r paste0(100*round(proc_14_18 %>% filter(Procedure != "Mifepristone Misoprostol") %>% group_by(Procedure) %>% summarise(tot = sum(Count)) %>% filter(Procedure == "Suction Curettage") %>% select(tot)/(proc_14_18 %>% ungroup() %>% filter(Procedure != "Mifepristone Misoprostol") %>% summarise(tot = sum(Count)) %>% select(tot)),3),"%")` of surgical abortions were suction curettage, `r paste0(100*round(proc_14_18 %>% filter(Procedure != "Mifepristone Misoprostol") %>% group_by(Procedure) %>% summarise(tot = sum(Count)) %>% filter(Procedure == "Dilation Evacuation") %>% select(tot)/(proc_14_18 %>% ungroup() %>% filter(Procedure != "Dilation Evacuation") %>% summarise(tot = sum(Count)) %>% select(tot)),3),"%")` were dilation and evacuation and the remaining fraction included both menstrual aspiration and 'other' categories. 
...
subtitle <- paste0(
  ...
  suction curettage, and dilation and evacuation as surgical.<br></span>")
#
med_lab <- paste0(
...
#
post_22_wk_lab <- paste0(
  "<span style = 'color:",body_text,";'>After 22 weeks (gestational age), abortion is legal
  only to prevent serious health risk to, or save the life of, the mother; or if the fetus has   been diagnosed with a lethal fetal anomaly.</span>")
...
```{r create_plot, fig.width=10, fig.height=6, fig.alt= "A bar chart of abortion procedure by gestational age from 2014 to 2017 showing 99% of all abortions happen by 13 weeks and medication abortions were about 30% of all."}
# assign plot to p 
...
theme(
  ...
  labs(
    title = title,
    subtitle  = subtitle,
    caption = "**Abortion Data:** 2014:2017 'www.in.gov/health/vital-records/vital-statistics/terminated-pregnancy-reports/'<br>
    **Graphic:** Ted Schurter 2023",
    alt = "A bar chart of abortion procedure by gestational age from 2014 to 2017 showing 99% of all abortions happen by 13 weeks weeks and medication abortions were about 30% of all."
  ) +
...
```{r, gestation data table}
gest <- read_csv("Exported_Data/raw_gest.csv")



datatable(gest,
          colnames = c("Year", "Gestational\nage in weeks", "Dilation\nand \nevacuation\nabortions", "Medication\nabortions", "Suction\ncurettage\nabortions", "Menstrual\naspiration\nabortions"),
          caption = htmltools::tags$caption(
            style = 'caption-side: top; text-align: center;',
            "Abortions in Indiana by year, gestational age and procedure type 2014 to 2017.", style = "color:black"),
          rownames=FALSE, 
          filter = "top", 
          extensions = c('Buttons', 'FixedColumns'),
          options = list(pageLength=10, 
  ...
  formatRound(c("de", "rx", "sc"), digits = 0)
```
...
# gp <- read_csv("Exported_Data/gest.csv")
# proc_tot <- read_csv("Exported_Data/proc_tot.csv")
# # calculate totals for gp df
# tot <- sum(gp[,3])
```


#### While the legality of Indiana's new abortion ban and the legal status of mifepristone remain unresolved, abortion in Indiana remains legal through 22 weeks of gestational age. Total abortion increased `r paste0(round(((proc_tot$total[proc_tot$year == max(proc_tot$year)]- proc_tot$total[proc_tot$year == min(proc_tot$year)]) / proc_tot$total[proc_tot$year == max(proc_tot$year)])*100,1),"%")` between 2014 and 2021 but has fluctuated as much as `r paste0(round(proc_tot %>%  mutate (pct_nrm = ((total-mean(total))/total)*100) %>% arrange(desc(pct_nrm))  %>% select(pct_nrm) %>%  slice(1),2),"%")` above and `r paste0(abs(round(proc_tot %>% mutate (pct_nrm = ((total-mean(total))/total)*100) %>%arrange(pct_nrm)  %>% select(pct_nrm) %>% slice(1),2)),"%")` below the `r prettyNum(round(mean(proc_tot$total)), big.mark = ",",   big.interval = 3L)` annual average. Over eight years, the annual percentage increase has averaged `r paste0(round(sum(proc_tot$pct_cng, na.rm=T)/length(proc_tot$year),2),"%")`. 

#### Much like the variation in annual abortion counts, monthly totals vary widely and swing as much as 20% from their eight-year average of `r round(mean(mnth$res))`. In 2019, September swung from `r mnth$res[mnth$year == 2019 & mnth$month == "September"]`, 18% below its average of `r mnth %>% filter(month == "September") %>% summarise(round(mean(res)))`, to `r round(mnth$res[mnth$year == 2020 & mnth$month == "September"])`, 11.7% above its average in 2020, a shift of 29.8%. 
...
```{r, calculate percents, plot pct cng mean, fig.width=10, fig.height=5.3, fig.alt="Line graph of Indiana resident monthly abortions totals 2014 to 2021 showing their departure from 8-year monthly average where September had the largest swing, changing 30% from 2019 to 2020."}
...
ggplot()+ theme_classic()+
  ...
  **Graphic:** Ted Schurter 2023",
  alt = "Line graph of Indiana resident monthly abortions totals 2014 to 2021 showing their departure from 8-year monthly average where September had the largest swing, changing 30% from 2019 to 2020.")  
```
...
mnth <- read_csv("Exported_Data/month_ab_clean.csv")


mnth %>% 
datatable(colnames = c("Year", "Month", "Resident\nabortions", "Non-resident\nabortions"),
  caption = htmltools::tags$caption(
          style = 'caption-side: top; text-align: center;',
    "Abortion totals in Indiana by year and month 2014 to 2021.", style = "color:black"),
          rownames=FALSE, 
          filter = "top", 
          extensions = c('Buttons', 'FixedColumns'),
          options = list(pageLength=10, 
  ...
  formatRound(c("res", "nonres"), digits = 0)
```
...
#### The variation is apparent when contrasting a month's totals to its eight-year average or to other years. 


```{r faceted month by year, fig.width=10, fig.height=5.3, fig.alt = "Small multiple line chart showing Indiana resident monthly abortion totals, average monthly totals, and average annual monthly totals 2014 to 2021"}
# set order of months
mnth$month <- factor(mnth$month, levels = c(
  "January", "February", "March", "April", "May", "June", "July","August",
  "September", "October", "November", "December"))
...
mnth %>% group_by(month) %>% 
  ...
  geom_line(data = mnth2,
            aes(m2, res, group = y2), color = "gray90", size = .2)+
  ...
  labs(title = paste0("The individual <span style = 'color:",avgc,"'>monthly average</span> of Indiana resident abortions from 2014 to 2021 reveals general trends 
  and how far a month’s<span style = 'color:",mc,"'> 
  totals</span> differ from its average in a given year."),
       ...
       caption = "<br>**Data:** 'www.in.gov/health/vital-records/vital-statistics/terminated-pregnancy-reports'<br>
  **Graphic:** Ted Schurter 2023",
  alt = "Small multiple line chart showing Indiana resident monthly abortion totals, average monthly totals, and average annual monthly totals 2014 to 2021")
```
...
#### Abortion rates also vary by county. `r str_to_sentence(english(round(per_cap %>% filter(annual_co_med < mean(per_cap$rate, na.rm = T)) %>% group_by(Name) %>% nrow()/(92*8)*100)))` Indiana counties out of `r per_cap %>% filter(year ==2014) %>% nrow()` had rates below the average county rate of `r round(mean(per_cap$rate, na.rm=T),2)` abortions per 1,000 women of childbearing age (10 to 49 years-old). At `r per_cap %>% arrange(desc(rate)) %>% select(Name, annual_co_med) %>% slice(1) %>% pull(2)`, `r per_cap %>% arrange(desc(rate)) %>% select(Name, annual_co_med) %>% slice(1) %>% pull(1)` County's average rate is `r round(per_cap %>% arrange(desc(annual_co_med)) %>% select(Name, rate) %>% slice(1) %>% pull(2)/mean(per_cap$rate, na.rm=T))` times higher than the state's county average; `r per_cap %>% arrange((annual_co_med)) %>% select(Name, annual_co_med) %>% slice(1) %>% pull(1)` County has the lowest average rate at `r per_cap %>% arrange((annual_co_med)) %>% select(Name, annual_co_med) %>% slice(1) %>% pull(2)` abortions per 1,000 women of childbearing age. 

##  {.tabset}

### Map
   
```{r fig.width = 8, fig.height=5, fig.align='center', fig.alt = "Choropleth map of abortion rate by Indiana county showing Marion County rate is at least 4 times higher than other counties."}
...
leaflet(sf_per_cap,
  ...
  addLegend(pal = pal, 
            values = ~county_average, 
            opacity = 1, 
            position = "bottomleft", 
            title = "Average abortion rate<br>per 1,000 females of<br> childbearing age 2014 to <br> 2021.<br><br>Click map to view county<br>rates.<br><br>",
            labels = c(0 == "zero")
            ) 
...
```{r, county count and rate data table}
per_cap <- per_cap %>% 
...
datatable(per_cap, 
          ...
          caption = htmltools::tags$caption(
            style = 'caption-side: top; text-align: center;',
    "Abortion rate in Indiana by year and county 2014 to 2021.", style = "color:black"),
          rownames=FALSE, 
          filter = "top", 
          extensions = c('Buttons', 'FixedColumns'),
          options = list(pageLength=10, 
...
#### The age at which women get abortions has remained fairly consistent since 2014. Women in their 20s get `r paste0(round(100*(ab_age %>% group_by(age) %>% filter(age == "20-24" | age == "25-29" ) %>% ungroup() %>%  summarise(sum(count)))/(ab_age %>% ungroup() %>% summarise(sum(count)))),"%")` of all abortions and those between 20 and 34 years-old get `r round(100*(ab_age %>% group_by(age) %>% filter(age == "20-24" | age == "25-29" | age == "30-34") %>% ungroup() %>%  summarise(sum(count)))/(ab_age %>% ungroup() %>% summarise(sum(count))))`%. Tracking changes in abortion among the very young is difficult due to inconsistencies in the reports' categorization of those under 16 years-old. In some years, the age ranges start as young as nine, while in others, all abortions for those under 16 are grouped in the same category. There was an average of `r round(ab_age %>% filter(age == "12-14" | age == "10-14" | age == "9-13") %>% ungroup() %>%  summarise(mean(count)))` abortions annually by nine to 14 year-olds, `r round(ab_age %>% filter(age == "12-14" | age == "10-14" | age == "9-13") %>% ungroup() %>%  summarise(sum(count))/ab_age %>%filter(year != 2020 & year != 2021) %>% ungroup() %>% summarise(sum(count))*100,2)`% of all abortions, between 2014 and 2019.  
...
```{r abortion by mother age, year, fig.width=10, fig.alt="Heat map of Indiana abortions by age 2014 to 2021 showing 20-34 year-old women get 77% of all abortions."}
ggplot(ab_age) + 
  ...
  labs(title = title,
       ...
       caption = "<br>**Data:** 'www.in.gov/health/vital-records/vital-statistics/terminated-pregnancy-reports'<br>
  **Graphic:** Ted Schurter 2023",
  alt = "Heat map of Indiana abortions by age 2014 to 2021 showing 20-34 year-old women get 77% of all abortions.")+
...
datatable(colnames = c("Year", "Age", "Count of\nabortions", "Percent\nby year"),
          caption = htmltools::tags$caption(
            style = 'caption-side: top; text-align: center;',
    "Abortion totals in Indiana by year, age of mother 2014 to 2021.", style = "color:black"),
...
# NEW ADDITION: CREATE COLUMN FOR CBA WOMEN
...
pop_ab_no_geo <- pop_ab

pop_ab_no_geo$geometry <- NULL
```

#### Though women in the 20 to 34 year-old age range get the most abortions, a high rate of women in that range within a county doesn't necessarily translate into a high county  abortion rate. Of the `r broman::spell_out(pop_ab %>% filter(bi_class == '3-3' | bi_class == '3-2' | bi_class == '3-1') %>% nrow(), capitalize = T)` counties with high rates of 20 to 34 year-old women in 2021, `r broman::spell_out(pop_ab %>% filter(bi_class == '3-3') %>% nrow(), capitalize = FALSE)` have high rates of abortion, `r broman::spell_out(pop_ab %>% filter(bi_class == '3-2') %>% nrow(), capitalize = FALSE)` have medium rates and `r broman::spell_out(pop_ab %>% filter(bi_class == '3-1') %>% nrow(), capitalize = FALSE)` have low rates. Among the `r broman::spell_out(pop_ab %>% filter(bi_class == '1-1' | bi_class == '1-2' | bi_class == '1-3') %>% nrow(), capitalize = T)` counties with low rates of 20 to 34 year-olds, `r broman::spell_out(pop_ab %>% filter(bi_class == '1-3') %>% nrow(), capitalize = FALSE)` has a high abortion rate, `r pop_ab %>% filter(bi_class == '1-2') %>% nrow()` have medium abortion rates and `r pop_ab %>% filter(bi_class == '1-1') %>% nrow()` have low abortion rates.
...
```{r create map, fig.alt="Bivariate map showing rates of 20 to 34 year-old women and the rate of abortion in 2021."}
map2 <- 
  ggplot() +
  geom_sf(data = pop_ab, mapping = aes(fill = bi_class), color = "white", size = 0.1, show.legend = FALSE) +
  bi_scale_fill(pal = "DkCyan", dim = 3)+
  ylim(37.85,41.75)+
  # low abortion low 20-34 curve ####
geom_curve(
  aes(
    x = ll_x/.992,
    xend = ll_x-.2,
  ...
  # low abortion low rate 20-34 text
  ...
  annotate("text", hl_x*.9875, hl_y*1.022,
           colour = "#6F7378",
           size = 3.15,
           label = "Blue counties:\nlow abortion, many\n20 to 34 year-olds.",
           lineheight = .9,
  ...
  labs(
    subtitle = paste0("Women in this age range are ", 
                      round(ab_age %>% filter(num %in% 4:6) %>% summarise(sum(pct)/8)),
    ...
    caption = paste0("<br>**Data:**  'www.in.gov/health/vital-records/files/2021-ITOP-Report.pdf'
   'www2.census.gov/programs-surveys/popest/datasets/2020-2021/counties/asrh/cc-est2021-agesex-18.csv'
   \n**Graphic:** Ted Schurter 2023",
   alt = "Bivariate map showing rates of 20 to 34 year-old women and the rate of abortion in 2021."))

finalPlot2 <- ggdraw() +
  draw_plot(map2, x =0, y= 0.015, width = 1, height = 1) +
  draw_plot(legend, x= 0.74, y = .115, width = 0.22, height = 0.22)+ # y=.13
  theme(
    plot.background = element_rect(fill = panel_c, color = panel_c)
  )
```
...
```{r, fig biv map, fig.height = 7.5, fig.alt="Bivariate map showing rates of 20 to 34 year-old women and the rate of abortion in 2021."}
finalPlot2
```
...
```{r age}
pop_ab_no_geo %>% 
  select(-c(GEOID, bi_x, bi_y, bi_class)) %>% 
  mutate(fem_rank = case_when(
    fem_rte < 160 ~ "Low-",
    fem_rte %in% 160:180 ~ "Medium",
    fem_rte > 180 ~ "High"))%>% 
  mutate(ab_rank = case_when(
    rate < 1.7 ~ "Low",
    rate > 1.71 & rate < 3.2 ~ "Medium",
    rate > 3.3 ~ "High")) %>% 
datatable(colnames = c("County", "20 to 34 year old\nfemale population", "Total\nfemale\npopulation", "Population of childbearing age females (10-49 years-old)", "Rate of\n20 to 34 year-old\nfemales per 1,000", "Count of\nabortions", "Abortion\nrate per 1,000", "Rank of rate of 20-24 year-olds",  "Rank of abortion rate"),
          caption = htmltools::tags$caption(
            style = 'caption-side: top; text-align: center;',
    "Average female population and abortion counts, and rates of 20 to 34 year-old women and abortion in Indiana from 2014 to 2021.", style = "color:black"),
  ...
  formatRound(c("fem_pop", "all_age", "cba_age", "Count"), digits = 0)
```
...
both_pop <- read_csv("Exported_Data/both_pop.csv") %>% 
  mutate(race_pct = 10*pct_race) # to get rate per 1,000

```

####  Between 2014 and 2021, the abortion rate for women of childbearing age in Indiana was `r round(sum(both_pop$count)/sum(both_pop$total)*1000,1)` per 1,000, though it varies by race. In that time, white women made up `r round(both_pop$avg_pct[both_pop$race == "White"])`% of the state's female population and received `r round(both_pop$ab_pct[both_pop$race == "White"])`% of all abortions, though they have the smallest population of childbearing age women, `r round(both_pop$cba_pct[both_pop$race == "White"])`%. The rate of abortion within that group is fourth-lowest among all races. Black women were on average `r round(both_pop$avg_pct[both_pop$race == "Black"])`% of the female population, had the second-lowest percentage of childbearing age women at `r round(both_pop$cba_pct[both_pop$race == "Black"])`%, and had the highest rate of abortion per 1,000 women of childbearing age at `r round(both_pop$race_pct[both_pop$race == "Black"])`%. Asians, the third-largest racial group making up `r round(both_pop$avg_pct[both_pop$race == "Asian"])`% of the population, had the highest percentage of childbearing age women, `r round(both_pop$cba_pct[both_pop$race == "Asian"])`%, and the third-highest abortion rate. 
...
more than three times the state average."),
    ...
    **Graphic:** Ted Schurter 2023",
alt = "Scatterplot of percent of childbearing age women by race and the rate of abortion within that population showing white women are the largest population, get the most abortions and have a low rate; Black women are the second-largest group and have the highest rate.")+
...
datatable(colnames = c("Population", "Percent of\ntotal population", "Childbearing age\npopulation",
          ...
          caption = htmltools::tags$caption(
            style = 'caption-side: top; text-align: center;',
    "Average race population and abortion totals in Indiana 2014 to 2021.", style = "color:black"),
...
#### Abortion is most prevalent among women who have not completed higher levels of education. Women with a high school diploma or GED made up between `r round(min(ed$Percent[ed$Level == "High school diploma or GED"]))`% and `r round(max(ed$Percent[ed$Level == "High school diploma or GED"]))`% of all abortions between 2014 and 2021. There were fewer abortions on average by women with a doctoral or professional degree, `r round(mean(ed$Percent[ed$Level == "Doctoral or professional degree"], na.rm = T),1)`%, than any other educational level.  


## {.tabset}

### Chart
```{r create line chart, fig.width=10, fig.height=5.75, fig.alt="Line chart of percent of abortion by education in Indiana from 2014 to 2021 showing women with a high school degree or GED and those with some college credit but no degree get the most abortions."}
# create line chart ####
...
top_ed_lab <- top_ed %>% filter(Year == 2021) %>% mutate(Level = case_match(Level,
  "High school diploma or GED" ~ "High school diploma\nor GED",
  "Some college credit, no degree" ~ "Some college credit,\nno degree"))
...
ggplot()+
  ...
  geom_text(data = top_ed_lab,
            aes(2021.25, Percent), color = ed_cols,
            label = top_ed_lab$Level, show.legend = F,
            vjust = 0, hjust = "left", lineheight =.9)+
  ...
  scale_y_continuous(labels = scales::percent_format(scale = 1),
                     name = "Percent of <br>all abortions")+
  t_theme()+
  ...
  labs(title = title,
       subtitle = subtitle,
       caption = caption, 
       alt = "Line chart of percent of abortion by education in Indiana from 2014 to 2021 showing women with a high school degree or GED and those with some college credit but no degree get the most abortions."
       )
...
datatable(colnames = c("Year", "Education Level", "Count of\nabortions", "Percent of\nabortions"),
  caption = htmltools::tags$caption(
          style = 'caption-side: top; text-align: center;',
    "Abortion totals in Indiana by year, education 2014 to 2021.", style = "color:black"),
          rownames=FALSE, 
          filter = "top", 
          extensions = c('Buttons', 'FixedColumns'),
          options = list(pageLength=10, 
...
```{r generate totals, calculate percents, fig.width=10}
...
#### The number of clinics that perform abortions in Indiana has hovered between `r min(prov$n[prov$type == "Facility"])` and `r max(prov$n[prov$type == "Facility"])` while the number of hospitals that performed at least one abortion annually fluctuates between `r min(prov$n[prov$type == "Hospital"])` and `r max(prov$n[prov$type == "Hospital"])`. Nearly all abortions occur in clinics.
...
mc_fac_lab <- c(
  "Affiliated Women's Services, Inc."            = "#708c9a", #a6cee3
...
```{r provider plot, fig.width=10, fig.alt="Line chart of count of abortion facility types (hospital or clinic) in Indiana from 2014 to 2021 showing the number of clinics is greater than hospitals every year."}
...
ggplot()+
  ...
  t_theme()+
  theme(
    panel.grid.major.y = element_blank(), #line(color = "gray93"),
    panel.background = element_rect(fill = panel_c),
    plot.background = element_rect(fill = panel_c),
    axis.title.x = element_blank(),
    axis.title.y = element_text(color = "gray55",
                                  size = rel(.6), vjust = .45, hjust = 1),
  )+
  labs(title = 
  ...
  'color:gray55'>total number</span> of facilities where abortions occurred peaked in 2021."),
       caption = caption, 
  alt = "Line chart of count of abortion facility types (hospital or clinic) in Indiana from 2014 to 2021 showing the number of clinics is greater than hospitals every year.")
```
...
datatable(prov %>% select(c(year, facility, county, count_by_yr, pct_by_yr)),
          ...
          colnames = c("Year", "Facility", "County", "Count of\nabortions by year",  "Percent of\nabortions by year"),
          caption = htmltools::tags$caption(
            style = 'caption-side: top; text-align: center;',
    "Abortion facility totals in Indiana by year 2014 to 2021.", style = "color:black"),
          rownames=FALSE, 
          filter = "top", 
          extensions = c('Buttons', 'FixedColumns'),
          options = list(pageLength=10, 
...
```{r, prov data frame}
# reimport prov (sf data frame from above not suitable - can't easily loose geometry column for caluclations...)

prov <- read_csv("Exported_Data/prov_ll.csv") 

#  total abortion count
tot_ab <- prov %>% summarise(tot = sum(count_by_yr)) %>% select(tot) %>% pull(1)
...
#### Marion County is home to `r english(nrow(prov %>% ungroup() %>% filter(type == "Facility" & county == "Marion") %>% distinct(facility)))` clinics and `r english(nrow(prov %>% ungroup() %>% filter(type == "Hospital" & county == "Marion") %>% distinct(facility)))` hospitals that performed at least one abortion per year. They accounted for `r round(100*prov %>% filter(county == "Marion") %>% ungroup() %>% summarise(sum(count_by_yr)/tot_ab),1)`% of all abortions. Facilities in Lake County accounted for `r round(100*prov %>% filter(county == "Lake") %>% ungroup() %>% summarise(sum(count_by_yr)/tot_ab),1)`%, of abortions, the second-highest percentage.
...
```{r create breakout county shapes and road details, cache=T}
...
im <- 
  ...
  alt = "Map showing location of abortion facilities in seven Indiana counties with circles showing percent of all abortions that occur in those facilities."
  )

```

```{r prov loc map,  fig.height = 7, fig.alt="Map showing location of abortion facilities in seven Indiana counties with circles showing percent of all abortions that occur in those facilities." }
im
```
...
prov <- read_csv("Exported_Data/prov_ll.csv") %>% group_by(year) %>% 
  mutate(pct_by_yr = 100*(count_by_yr/sum(count_by_yr)),
         type = if_else(
           str_detect(facility, "Hospital") == TRUE,
           "Hospital", "Facility"
...
#### `r prov %>% ungroup() %>% arrange(desc(tot_fac_pct)) %>% select(facility) %>% slice(1)` in `r prov %>% ungroup() %>% arrange(desc(tot_fac_pct)) %>% select(county) %>% slice(1)` County performed the most abortions in the state.


```{r facility count by year, fig.width=10, fig.alt="Line chart of percent of abortions by clinics in Indiana from 2014 to 2021 showing clinics in Marion County performed most abortions in Indiana."}
...
ggplot()+
  ...
  labs(title = 
         paste0("There were ", prov %>% ungroup() %>%  filter(type == "Facility") %>% 
               ...
               in ", english(prov %>% ungroup() %>%  filter(type == "Facility") %>% 
       ...
       caption = caption,
       alt = "Line chart of percent of abortions by clinics in Indiana from 2014 to 2021 showing clinics in Marion County performed most abortions in Indiana.")
```

---

#### `r prov %>% filter(type == "Hospital") %>% ungroup() %>% arrange(desc(tot_fac_pct)) %>% select(facility) %>% slice(1)` in `r prov %>% filter(type == "Hospital") %>% ungroup() %>% arrange(desc(tot_fac_pct)) %>% select(county) %>% slice(1)` County performed more abortions than any other Indiana hospital but only a  fractional percent of all abortions.


```{r, hospital count by year, fig.width=10, fig.alt="Line chart of percent of abortions in Indiana from 2014 to 2021 that occured in hospitals showing less than 1% of abortions occured in hospitals and Sidney & Lois Eskenazi Hospital accounted for 63% of them."}
ggplot()+
  ...
  labs(title = 
         paste0("Abortions in hospitals comprise just ", 
             ...
             was one of ",
                english(prov %>% ungroup() %>% filter(type == "Hospital") %>% distinct(facility) %>% 
       ...
       subtitle = paste0("There were <span style = 'color:#756bb1;'>",
                         english(prov %>% ungroup() %>% filter(count_by_yr <= 1 & type == "Hospital") %>% distinct(facility) %>% 
                           select(facility) %>% nrow()),"</span> 
                         instances when hospitals performed a <span style = 'color:#756bb1;'>single</span> abortion in a year."),
       caption = caption,
       alt = "Line chart of percent of abortions in Indiana that occured in hospitals by the hospitals showing less than 1% of abortions occured in hospitals and Sidney & Lois Eskenazi Hospital accounted for 63% of them.")
```

---

```{r, import nonresident data}
nres <- read_csv("Exported_Data/nonresident_ab_19_21.csv")
```


#### Indiana began reporting the number of residents from nearby states who received abortions in Indiana in 2019. Of the five listed, `r nres %>% group_by(state) %>% summarise(tot = sum(abortions)) %>% arrange(desc(tot)) %>% slice(1) %>% pull(1)` had the most with `r nres %>% group_by(state) %>% summarise(tot = sum(abortions)) %>% arrange(desc(tot)) %>% slice(1) %>% pull(2)`, four times as many as `r nres %>% group_by(state) %>% summarise(tot = sum(abortions)) %>% arrange(desc(tot)) %>% slice(2) %>% pull(1)`, the next highest state. 

## {.tabset}

### Chart 

```{r, update t_theme for combo plot - patchwork conflict in Rmd}
t_theme <- function(base_size = 10){
...
```{r, non-resident abortion data table}
nres <- read_csv("Exported_Data/nonresident_ab_19_21.csv")
...
nres %>% 
  datatable(
          ...
          caption = htmltools::tags$caption(
            style = 'caption-side: top; text-align: center;',
    "Abortion totals for non-residents 2019 to 2021.", style = "color:black"),
          rownames=FALSE, 
          filter = "top", 
          extensions = c('Buttons', 'FixedColumns'),
          options = list(pageLength=10, 
...
#### New laws and legislation are likely to continue altering the legal landscape of abortion in Indiana. The degree to which the state's short-lived ban impacted 2022 abortion totals won't be clear until the IDH releases the year's full data. As the legal fight over Indiana's abortion law continues, so too do battles over bans in neighboring [Kentucky](https://apnews.com/article/abortion-ban-kentucky-lawsuit-141c930e1ca71bcb5833c5635814a3dd) and [Ohio](https://apnews.com/article/abortion-health-ohio-government-and-politics-d4dd3a0c4816ccb9f56df30163f58b9e), states whose residents' abortions are often tallied in Indiana's reports. Where and how those residents access abortion services, and how the federal fate of mifepristone plays out are just a few of the unknown issues with potentially large impacts. The near-term future of abortion in Indiana is unknown, but it's likely to be different than its recent past. 

---

###### Data and analysis workflow available at [https://github.com/tedschurter/indiana_abortion](https://github.com/tedschurter/indiana_abortion)
