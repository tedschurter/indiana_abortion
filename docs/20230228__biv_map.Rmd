...
<br><br>
The method to divide the rate into three parts that seems to best match the abortion rate data is to use thirds after removing outliers calculated using the 1.5 times the interquartile range method. <br><br>
...
inc_ab <- inc_ab %>% mutate(rank = paste0(ab_f_trd_rank, in_jnks),
                            .after = Name)
```
...
```{r, add markers for abortion providers, echo=T}
# import provider locations and filter to 2017:2021
prov <- read_csv("Exported_Data/prov_counties.csv") %>% 
  filter(year %in% 2017:2021) %>% 
  distinct(county)

# get geometry from sf_biv (it's already) and join

prov <- left_join(prov, (sf_biv %>% 
                           filter(Name %in% c(prov$county)) %>% 
                           select(Name, geometry) %>% 
                           distinct(Name, geometry)), by = c("county" = "Name"))

# calculate centroid for each county
prov <- prov %>% mutate(
  centroid = st_centroid(prov$geometry))
...
lali_county <-  sf_biv %>% filter(year == 2021) %>% 
  ...
  filter(rank == "lali") %>% 
  slice(5) %>%  # can adjust slice to select needed la-li county 
...
p <- 
  ...
  geom_sf(data = sf_biv %>% filter(year == 2021),
          ...
          alpha = .95,
          show.legend = F)+
  geom_sf(data=prov$centroid,
          size = 2.5, shape = 23, fill = panel_c,
          show.legend = T)+
  scale_fill_identity()+
...
geom_curve( 
  ...
  annotate("text", lali_county_x/.977, lali_county_y*1.008,
           colour = "#6F7378",
           size = 3.5,
  ...
  annotate("text", hahi_county_x*1.027, hahi_county_y/1.007,
           colour = "#6F7378",
           size = 3.5,
  ...
  annotate("text", hali_county_x*.992, hali_county_y/.998,
           colour = "#6F7378",
           size = 3.5,
           label = "Blue counties\nmean high abortion\nhave low income",
           lineheight = .9,
  ...
  geom_curve( 
    aes(
      x = lahi_county_x/1.012,
      xend = lahi_county_x,
      y = lahi_county_y*1.001,
      yend = lahi_county_y),
    curvature = .1,
    linewidth = .75,
    color = "#6F7378",
    arrow = arrow(
      length = unit(.2, "cm")))+
  # low abortion, high income county curve  
  annotate("text", lahi_county_x/1.0084, lahi_county_y*.9964,
           colour = "#6F7378",
           size = 3.5,
